...
function rafraichirListe(filtre = '') {
  ...
  convs.forEach(c => {
    ...
    div.dataset.id = c.id;
    const heure = c.timestampDernier ? new Date(c.timestampDernier).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'}) : '';
    const plIco = c.jeuPlateforme === 'pc' ? 'computer" class="fa-solid' : `${c.jeuPlateforme}" class="fa-brands`;
    div.innerHTML = `
      <div class="conv-entete">
        <div class="conv-plateforme ${c.jeuPlateforme}"><i class="fa-${plIco} fa-${c.jeuPlateforme === 'pc' ? 'computer' : c.jeuPlateforme}"></i></div>
...
function afficherMessageAdmin(type, texte, timestamp, auteur) {
  ...
  const heure      = timestamp ? new Date(timestamp).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'}) : '';
  const div = document.createElement('div');
...
window.envoyerReponse = async () => {
  ...
  // Marque la conv comme lue et met à jour le dernier message
...
window.gererToucheAdmin = e => { if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();envoyerReponse();} };
